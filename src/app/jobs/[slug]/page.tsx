      ...
      <main className="bg-surface">
        <div className="max-w-6xl mx-auto px-5 py-8 md:py-12">
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <Link key={opp.href} href={opp.href} className="text-[12px] font-medium px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 border border-amber-100 hover:bg-amber-100 transition-colors">
                  {opp.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {nearby.map((c) => (
                <Link key={c} href={`/jobs/in-${slugifyCounty(c)}`} className="text-[12px] font-medium px-3 py-1.5 rounded-lg bg-ink/[0.04] text-ink/70 hover:bg-ink/[0.08] hover:text-ink transition-colors">
                  {c}
                </Link>
              ))}
            </div>
    ...
    <main className="bg-surface">
      ...
      <div className="border-b border-divider bg-white/60 backdrop-blur-sm sticky top-0 z-30">
        <div className="max-w-3xl mx-auto px-5 py-3 flex items-center gap-3">
          <Link href="/" className="inline-flex items-center gap-1 text-[13px] text-muted hover:text-ink transition-colors">
      ...
      <article className="max-w-3xl mx-auto px-5 py-8 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {job.governmentLevel && (
            <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-blue-50 text-blue-700 border border-blue-100">
              {job.governmentLevel === "NATIONAL" ? "National Government" : job.governmentLevel === "COUNTY" ? "County Government" : "State Corporation"}
            </span>
          )}
          {job.opportunityType && (
          ...
          {job.tags && job.tags.map((tag: string) => (
            <span key={tag} className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-surface text-muted border border-subtle">#{tag}</span>
          ))}
        </div>
        ...
        <div className="rounded-lg bg-emerald-50/60 border border-emerald-100/80 px-5 py-4">
          ...
          {job.salaryMin != null && job.salaryMax != null ? (
            <p className="text-base font-semibold text-ink">
              ...
              {job.salaryPeriod && <span className="font-normal text-muted"> / {job.salaryPeriod}</span>}
            </p>
          ) : (
            <>
              <p className="text-base font-medium text-ink/60">{job.predictedSalary || "Not disclosed"}</p>
              {job.predictedSalary && <p className="text-[12px] text-muted mt-1">Estimated based on similar roles in {job.county || job.location}</p>}
            </>
          )}
        </div>
        ...
        {job.description && (
          <div>
            ...
            <div
              className="text-[14px] text-ink/80 leading-relaxed [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:mb-1.5 [&_p]:mb-3 [&_strong]:font-semibold [&_strong]:text-ink"
        ...
        <div className="pt-4 border-t border-divider flex items-center justify-between gap-4">
          ...
          {job.applicationUrl ? (
            <a href={job.applicationUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-ink text-white text-[14px] font-medium hover:bg-ink/90 transition-colors">
