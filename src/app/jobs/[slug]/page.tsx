...
export default async function SlugPage({
  ...
  if (slug.startsWith(COUNTY_PREFIX)) {
    ...
    const [countResult, nearby, relatedCategories] = await Promise.all([
      getJobsByCounty(countySlug, 20).catch(() => ({ jobs: [] as any[], count: 0 })),
          ...
          {countyJobs.length > 0 ? (
            <div className="mb-10">
              ...
              <div className="divide-y divide-subtle">
                {countyJobs.map((job) => {
                  const dl = job.deadline ? Math.ceil((new Date(job.deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : null;
                  const urgent = dl !== null && dl <= 3 && dl > 0;
                  return (
                    <JobRowClickable key={job.id} slug={job.slug} className="grid grid-cols-12 gap-4 py-3.5 group cursor-pointer hover:bg-ink/[0.02] rounded-lg -mx-2 px-2 transition-colors">
                      ...
                      <div className="hidden sm:block sm:col-span-3 text-[12px] text-muted truncate">{job.companyName}</div>
                      <div className="col-span-6 sm:col-span-2 flex items-center">
                        <span className="text-[11px] text-muted">{job.listingType === "GOVERNMENT" ? "Gov" : job.listingType === "CASUAL" ? "Casual" : job.employmentType || "Job"}</span>
                      </div>
                      <div className="col-span-6 sm:col-span-2 flex sm:justify-end items-center">
                        {dl !== null ? (
                          <span className={`font-mono text-[12px] font-medium tabular-nums ${dl <= 0 ? "text-muted/40" : urgent ? "text-accent" : "text-muted"}`}>{dl <= 0 ? "Closed" : `${dl}d left`}</span>
              ...
              {count > 20 && <Link href={`/jobs?county=${countySlug}`} className="inline-flex items-center gap-1 text-[13px] font-medium text-accent hover:text-accent-dark transition-colors mt-4">View all {count} jobs in {countyName} →</Link>}
            </div>
          ) : (
            <div className="mb-10"><RichFallback county={countyName} listingCount={count} nearbyCounties={nearby} relatedCategories={relatedCategories} /></div>
          )}
          <div className="mb-10">
            <h2 className="text-[13px] font-semibold text-ink uppercase tracking-wider mb-4">Jobs in {countyName} by Category</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {JOB_CATEGORIES.slice(0, 12).map((cat) => (
                <Link key={cat.slug} href={`/jobs/category/${cat.slug}/in-${countySlug}`} className="flex items-center justify-between px-4 py-3 rounded-lg border border-divider hover:border-accent/30 hover:bg-accent-bg/50 transition-all group">
                  <span className="text-[13px] font-medium text-ink/80 group-hover:text-ink">{cat.label}</span>
                  <svg className="w-4 h-4 text-muted group-hover:text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </Link>
              ))}
            </div>
          </div>
          <div className="mb-10">
            <h2 className="text-[13px] font-semibold text-ink uppercase tracking-wider mb-4">Opportunities in {countyName}</h2>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <Link key={opp.href} href={opp.href} className="text-[12px] font-medium px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 border border-amber-100 hover:bg-amber-100 transition-colors">{opp.label}</Link>
              ))}
            </div>
          </div>
          <div>
            <h2 className="text-[13px] font-semibold text-ink uppercase tracking-wider mb-4">Nearby Counties</h2>
            <div className="flex flex-wrap gap-2">
              {nearby.map((c) => (
                <Link key={c} href={`/jobs/in-${slugifyCounty(c)}`} className="text-[12px] font-medium px-3 py-1.5 rounded-lg bg-ink/[0.04] text-ink/70 hover:bg-ink/[0.08] hover:text-ink transition-colors">{c}</Link>
              ))}
            </div>
          </div>
        </div>
    ...
    <main className="bg-surface">
      ...
      <article className="max-w-3xl mx-auto px-5 py-8 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px] text-muted">
          ...
          {job.deadline && <span className="text-accent font-medium">Deadline: {formatDateUTC(job.deadline)}</span>}
        </div>
        <div className="flex flex-wrap gap-2">
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-ink/[0.05] text-ink/70">
            {job.listingType === "JOB" ? "Job" : job.listingType === "GOVERNMENT" ? "Government" : job.listingType === "CASUAL" ? "Casual" : "Opportunity"}
          </span>
          {job.governmentLevel && <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-blue-50 text-blue-700 border border-blue-100">{job.governmentLevel === "NATIONAL" ? "National Government" : job.governmentLevel === "COUNTY" ? "County Government" : "State Corporation"}</span>}
          {job.opportunityType && <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-amber-50 text-amber-700 border border-amber-100">{job.opportunityType.replace(/_/g, " ")}</span>}
          {job.tags && job.tags.map((tag: string) => <span key={tag} className="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-medium bg-surface text-muted border border-subtle">#{tag}</span>)}
        </div>
        <div className="rounded-lg bg-emerald-50/60 border border-emerald-100/80 px-5 py-4">
          <p className="text-[11px] font-medium text-emerald-600 uppercase tracking-wider mb-1.5">{job.listingType === "OPPORTUNITY" ? "Value" : "Compensation"}</p>
          {job.salaryMin != null && job.salaryMax != null ? (
            <p className="text-base font-semibold text-ink">KES {job.salaryMin?.toLocaleString()} – {job.salaryMax?.toLocaleString()}{job.salaryPeriod && <span className="font-normal text-muted"> / {job.salaryPeriod}</span>}</p>
          ) : (
            <><p className="text-base font-medium text-ink/60">{job.predictedSalary || "Not disclosed"}</p>{job.predictedSalary && <p className="text-[12px] text-muted mt-1">Estimated based on similar roles in {job.county || job.location}</p>}</>
          )}
        </div>
        {job.description && (
          <div>
      ...
      <div className="max-w-3xl mx-auto px-5 pb-8"><AdSlot format="auto" style={{ display: 'block', minHeight: '90px' }} /></div>
    </main>
  );
}
