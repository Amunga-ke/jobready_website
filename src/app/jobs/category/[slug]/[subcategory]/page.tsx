...
import { slugifyCounty, getCountyBySlug, KE_COUNTIES } from "@/lib/constants";
 ...
 * Uses KE_COUNTIES list for reliable disambiguation.
 */
...
export async function generateMetadata({
  ...
  try {
    const { slug, subcategory: seg } = await params;
    const loc = parseSegment(seg);
...
export default async function CategorySecondSegmentPage({
  params,
}: {
  params: Promise<{ slug: string; subcategory: string }>;
}) {
  try {
    ...
    if (loc) {
      return <CategoryCountyView slug={slug} countySlug={loc.countySlug} county={loc.county} />;
    }
...
async function CategoryCountyView({
  slug,
  countySlug,
  county,
}: {
      ...
      <div className="max-w-6xl mx-auto px-5 py-8 md:py-12">
        <SeoPageHeader
          breadcrumbs={[
            ...
            { label: county, href: `/jobs/category/${slug}/in-${countySlug}` },
          ]}
          title={`${category.name} Jobs in ${county}`}
          description={`Browse ${category.name.toLowerCase()} job openings in ${county}, Kenya.`}
        ...
        {listings.length > 0 ? (
          ...
          <div className="mb-10">
            <RichFallback county={county} listingCount={count} nearbyCounties={KE_COUNTIES.slice(0, 8) as unknown as string[]} />
          </div>
        )}

        {/* Subcategories in this county */}
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              {subcategories.map((sub) => (
                <Link
                  key={sub.slug}
                  href={`/jobs/category/${slug}/${sub.slug}/in-${countySlug}`}
        ...
        <div className="flex flex-wrap gap-3">
          <Link href={`/jobs/category/${slug}`} className="text-[13px] font-medium text-accent hover:text-accent-dark transition-colors">
            &larr; All {category.name} jobs
          </Link>
          <Link href={`/jobs/in-${countySlug}`} className="text-[13px] font-medium text-accent hover:text-accent-dark transition-colors">
...
async function SubcategoryView({ slug, subSlug }: { slug: string; subSlug: string }) {
  const category = await prisma.category
    .findUnique({ where: { slug } })
    .catch(() => null);
  if (!category) notFound();
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              {siblings.map((s) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {s._count.listings > 0 && (
                      <span className="text-[11px] font-mono text-accent">{s._count.listings}</span>
                    )}
                    <svg className="w-4 h-4 text-muted group-hover:text-accent transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
...
// ─── Shared Job Listing Row ─────────────────────────────────────────────

function JobListingRow({ job }: { job: any }) {
