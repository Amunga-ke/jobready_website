...
export async function generateMetadata({
  ...
  const title = `${category.name} Jobs in ${countyName}${count > 0 ? ` (${count})` : ""} | JobReady`;
      ...
      <div className="max-w-6xl mx-auto px-5 py-8 md:py-12">
        <SeoPageHeader
          breadcrumbs={[
            ...
            { label: countyName, href: `/jobs/category/${slug}/in-${countySlug}` },
          ]}
          title={`${category.name} Jobs in ${countyName}`}
          description={`Browse ${category.name.toLowerCase()} job openings in ${countyName}, Kenya.`}
        ...
        {listings.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="col-span-6 sm:col-span-2 flex items-center">
                      <span className="text-[11px] text-muted">
                        {job.subcategory?.name || job.employmentType || job.listingType}
                      </span>
                    </div>
          ...
          <div className="mb-10 text-center py-12">
            <p className="text-[14px] text-muted">
              No {category.name.toLowerCase()} jobs in {countyName} at the moment. Check back soon.
            </p>
          </div>
        )}
